...
class ExtsysAdhocCommand extends CConsoleCommand {
	public function run($args) {
		...
		try {
			$sql = "select a.username, b.disp_name,ifnull(b.email,'') as emailaddr
					from security.sec_user_access a, security.sec_user b
					where a.system_id='acct' and a.username=b.username and b.status='A' and b.email<>'' and b.email is not null
					and ((a.a_read_only is not null and a.a_read_only <>'') or (a.a_read_write is not null and a.a_read_write<>''))
				";

			$rows = Yii::app()->db->createCommand($sql)->queryAll();
			foreach ($rows as $row) {
				echo $row['username'].' / '.$row['disp_name'].' / '.$row['emailaddr']."\n";
				$oparam = json_encode(array(
						'role'=>array(), 'user'=>'', 'right'=>'NA', 'disp_name'=>'', 'email'=>''
					));

				$nparam = json_encode(array(
						'role'=>array('Finance_Team'), 'user'=>$row['emailaddr'], 'right'=>'CN', 
				...
				if (strpos($sql,':username')!==false)
					$command->bindParam(':username',$row['emailaddr'],PDO::PARAM_STR);
